...
echo "Creating MySQL user with full permissions..."
$MYSQL -u "$MYSQL_USERNAME" -p"$MYSQL_PASSWORD" -e "CREATE USER '$NEW_USERNAME'@'%' IDENTIFIED BY '$NEW_PASSWORD';" 2>/dev/null
$MYSQL -u "$MYSQL_USERNAME" -p"$MYSQL_PASSWORD" -e "GRANT ALL PRIVILEGES ON $DATABASE.* TO '$NEW_USER'@'%';" 2>/dev/null
$MYSQL -u "$MYSQL_USERNAME" -p"$MYSQL_PASSWORD" -e "FLUSH PRIVILEGES;" 2>/dev/null

echo "MySQL user '$NEW_USERNAME' created with full permissions for database '$DATABASE'."
